...
class MyWorld(pydart.World):
    ...
    def build_ik(self, ):
        ik = IK()
        ik.set_skeleton(self.robot)
        import time
        for i, m in enumerate(self.robot.markers):
            body = m.bodynode
            offset = m.local_position()
            ik.add_body_and_offset(body, offset)
            assert(np.allclose(body.to_world(offset), m.world_position()))
    ...
    def set_frame(self, frame_index):
        ...
        self.ik.set_targets(frame_data)
    ...
    def f(self, x):
        ...
        #     # print(w * np.linalg.norm(lhs - rhs) ** 2, lhs, rhs)
    ...
    def solve(self, ):
        ...
        if method == 0:
            ...
            tic = time()
            self.ik.set_verbose(False)
            self.ik.solve(1e-3)
            toc = time()
            # print("elapsed = %.8fs" % (toc - tic))
